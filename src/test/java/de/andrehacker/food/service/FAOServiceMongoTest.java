...
//    System.out.println("PerItem: " + json);
    mockMvc.perform(get("/peritem?itemid=667&measureid=5510&year=2011").accept(MediaType.parseMediaType("application/json;charset=UTF-8")))
    .andExpect(status().isOk())
    .andExpect(content().contentType("application/json;charset=UTF-8"))
    .andExpect(jsonPath("$", hasSize(greaterThan(1))))
...
//    System.out.println("PerCountry: " + json);
    mockMvc.perform(get("/percountry?countryid=351&measureid=5510&year=2011").accept(MediaType.parseMediaType("application/json;charset=UTF-8")))
    .andExpect(status().isOk())
    .andExpect(content().contentType("application/json;charset=UTF-8"))
    .andExpect(jsonPath("$", hasSize(greaterThan(1))))
  ...
  public void testPerCountryAndTime() throws Exception {
    ...
    mockMvc.perform(get("/percountrytime?countryid=351&measureid=5510&items=667,656&yearfrom=2010&yearto=2011").accept(MediaType.parseMediaType("application/json;charset=UTF-8")))
    .andExpect(status().isOk())
    .andExpect(content().contentType("application/json;charset=UTF-8"))
    .andExpect(jsonPath("$", hasSize(greaterThan(1))))
